...
type ConfigTab = 'mcal' | 'bsw';

/* ---------- MCAL Module Definitions ---------- */
interface McalParam {
  name: string;
  label: string;
    ...
    <section className="py-16 bg-card/30 border-y border-border">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="text-center mb-10">
          <span className="inline-flex items-center gap-2 px-4 py-1.5 rounded-full bg-[hsl(var(--accent))]/10 text-[hsl(var(--accent))] text-sm font-medium mb-4">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          {/* Left: Module Selection & Parameters */}
          <div className="bg-card border border-border rounded-2xl p-6">
            {activeTab === 'mcal' ? (
              <>
                {/* MCAL Module Selector */}
                <div className="mb-6">
                  <label className="block text-sm font-medium mb-3">选择 MCAL 模块</label>
                  <div className="grid grid-cols-2 sm:grid-cols-3 gap-2">
                    {mcalModules.map((mod) => (
                      <button
                        key={mod.id}
                        onClick={() => setSelectedMcalModule(mod.id)}
                        className={`flex items-center gap-2 px-3 py-2 rounded-lg text-sm transition-all ${
                          selectedMcalModule === mod.id
                ...
                <div className="space-y-4">
                  <div className="flex items-center gap-2 pb-3 border-b border-border">
                    <currentMcalModule.icon className="w-5 h-5 text-[hsl(var(--accent))]" />
                    <h3 className="font-semibold">{currentMcalModule.name}</h3>
                  </div>
                  {currentMcalModule.params.map((param) => (
                    <div key={param.name} className="space-y-1.5">
                      <label className="text-sm font-medium">{param.label}</label>
                      {param.type === 'select' ? (
                        <div className="relative">
                          <select
                            value={String(mcalValues[selectedMcalModule]?.[param.name] ?? param.defaultValue)}
                            onChange={(e) => handleMcalChange(param.name, e.target.value)}
                            className="w-full px-3 py-2 rounded-lg bg-muted border border-border text-sm appearance-none focus:outline-none focus:ring-2 focus:ring-[hsl(var(--accent))]/50"
                          >
                        ...
                        <button
                          onClick={() => handleMcalChange(param.name, !mcalValues[selectedMcalModule]?.[param.name])}
                        ...
                        >
                          {mcalValues[selectedMcalModule]?.[param.name] ? '已启用' : '已禁用'}
                        </button>
                      ) : (
                        <input
                          type={param.type === 'number' ? 'number' : 'text'}
                          value={String(mcalValues[selectedMcalModule]?.[param.name] ?? '')}
                          onChange={(e) => handleMcalChange(param.name, param.type === 'number' ? Number(e.target.value) : e.target.value)}
                          className="w-full px-3 py-2 rounded-lg bg-muted border border-border text-sm focus:outline-none focus:ring-2 focus:ring-[hsl(var(--accent))]/50"
                        />
                      )}
                ...
                <button
                  onClick={handleGenerateMcal}
                  className="w-full mt-6 flex items-center justify-center gap-2 px-4 py-3 bg-[hsl(var(--primary))] text-primary-foreground rounded-xl font-medium hover:bg-[hsl(var(--primary-glow))] transition-all"
                >
              ...
              <>
                {/* BSW Module Selector */}
                <div className="mb-6">
                  <label className="block text-sm font-medium mb-3">选择 BSW 模块</label>
                  <div className="grid grid-cols-2 sm:grid-cols-3 gap-2">
                    {bswModules.map((mod) => (
                      <button
                        key={mod.id}
                        onClick={() => setSelectedBswModule(mod.id)}
                        className={`flex items-center gap-2 px-3 py-2 rounded-lg text-sm transition-all ${
                          selectedBswModule === mod.id
                            ? 'bg-[hsl(var(--primary))] text-primary-foreground'
                            : 'bg-muted text-muted-foreground hover:text-foreground'
                        }`}
                      >
                ...
                <div className="space-y-4">
                  <div className="flex items-center gap-2 pb-3 border-b border-border">
                    <currentBswModule.icon className="w-5 h-5 text-[hsl(var(--accent))]" />
                    <div>
                      <h3 className="font-semibold">{currentBswModule.name}</h3>
                      <span className="text-xs text-muted-foreground">{currentBswModule.layer} Layer</span>
                    </div>
                  </div>
                  {currentBswModule.params.map((param) => (
                    <div key={param.name} className="space-y-1.5">
                      <label className="text-sm font-medium">{param.label}</label>
                      {param.type === 'select' ? (
                        <div className="relative">
                          <select
                            value={String(bswValues[selectedBswModule]?.[param.name] ?? param.defaultValue)}
                            onChange={(e) => handleBswChange(param.name, e.target.value)}
                            className="w-full px-3 py-2 rounded-lg bg-muted border border-border text-sm appearance-none focus:outline-none focus:ring-2 focus:ring-[hsl(var(--accent))]/50"
                          >
                            {param.options?.map((opt) => (
                              <option key={opt} value={opt}>{opt}</option>
                        ...
                        <button
                          onClick={() => handleBswChange(param.name, !bswValues[selectedBswModule]?.[param.name])}
                          className={`w-full px-3 py-2 rounded-lg text-sm font-medium transition-all ${
                            bswValues[selectedBswModule]?.[param.name]
                              ? 'bg-emerald-500/10 text-emerald-500 border border-emerald-500/20'
                              : 'bg-muted text-muted-foreground border border-border'
                          }`}
                        >
                          {bswValues[selectedBswModule]?.[param.name] ? '已启用' : '已禁用'}
                        </button>
                      ) : (
                        <input
                          type={param.type === 'number' ? 'number' : 'text'}
                          value={String(bswValues[selectedBswModule]?.[param.name] ?? '')}
                          onChange={(e) => handleBswChange(param.name, param.type === 'number' ? Number(e.target.value) : e.target.value)}
                          className="w-full px-3 py-2 rounded-lg bg-muted border border-border text-sm focus:outline-none focus:ring-2 focus:ring-[hsl(var(--accent))]/50"
                        />
                      )}
                      <p className="text-xs text-muted-foreground">{param.desc}</p>
          ...
          <div className="bg-card border border-border rounded-2xl p-6">
            <div className="flex items-center justify-between mb-4">
              <div className="flex items-center gap-2">
                <Code className="w-5 h-5 text-[hsl(var(--accent))]" />
                <h3 className="font-semibold">生成的配置代码</h3>
              </div>
              <div className="flex items-center gap-2">
                <button
                  onClick={handleCopy}
                  disabled={!generatedCode}
                  className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-muted text-sm font-medium hover:bg-muted/80 transition-all disabled:opacity-50 disabled:cursor-not-allowed"
                >
                ...
                <button
                  ...
                  disabled={!generatedCode}
                  className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-[hsl(var(--primary))] text-primary-foreground text-sm font-medium hover:bg-[hsl(var(--primary-glow))] transition-all disabled:opacity-50 disabled:cursor-not-allowed"
                >
            ...
            <div className="relative">
              <pre className="h-[500px] overflow-auto bg-muted rounded-xl p-4 text-xs font-mono text-foreground">
                {generatedCode || (
                  <span className="text-muted-foreground">
                    {/* Placeholder */}
                    {`/* \u70b9\u51fb\u300c\u751f\u6210\u914d\u7f6e\u4ee3\u7801\u300d\u6309\u94ae\u67e5\u770b\u751f\u6210\u7684 C \u5934\u6587\u4ef6 */
